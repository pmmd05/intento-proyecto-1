...
def create_playlist(access_token: str, emotion: str, track_uris: list) -> Dict:
    """
    Crea una playlist en Spotify con las canciones recomendadas

    Args:
        access_token: Token de acceso de Spotify
        emotion: Emoción detectada (para el nombre de la playlist)
        track_uris: Lista de URIs de Spotify de las canciones
    ...
    try:
        ...
        # 2. Mapeo de emociones a nombres en español
        emotion_names = {
            "happy": "Feliz",
            "sad": "Triste",
            "angry": "Enojado",
        ...
        emotion_display = emotion_names.get(emotion.lower(), emotion.capitalize())

        # 3. Crear la playlist
        create_url = f"{SPOTIFY_API_BASE_URL}/users/{user_id}/playlists"
        playlist_data = {
            "name": f"Ánima - {emotion_display}",
            "description": f"Playlist generada por Ánima basada en tu estado de ánimo: {emotion_display}",
            "public": False
        }
        ...
        if not playlist_id:
            return {
                ...
                "message": "No se recibió el ID de la playlist"
            }

        # 4. Agregar canciones a la playlist (máximo 100 por request)
        if track_uris:
            # Spotify permite máximo 100 canciones por request
            ...
            for chunk in chunks:
                ...
                if add_response.status_code not in [200, 201]:
                    print(f"⚠️ Error agregando canciones al playlist: {add_response.status_code}")
        ...
        return {
            "success": False,
            "error": "timeout",
            "message": "Timeout al conectar con Spotify"
        }
    except Exception as e:
